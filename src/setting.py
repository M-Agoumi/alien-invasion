    ...
    def draw_stars(self):
        if self.stars:
            ...
            for i, (x, y) in enumerate(self.stars):
                ...
                if y > self.get_screen_y():
                    self.stars[i] = (random.randint(0, self.get_screen_x()), -20)
            ...
            for _ in range(100):  # You can adjust the number of stars as desired
                x = random.randint(0, self.get_screen_x())  # Random x-coordinate within the screen width
                y = random.randint(0, self.get_screen_y())  # Random y-coordinate within the screen height
                stars.append((x, y))
                self.screen.blit(self.star, [x, y])
            self.stars = stars
